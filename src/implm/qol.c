...
{
    if (beg < 0 || end < 0 || beg > vec->size || end > vec->size || beg > end)
        { // if requested size is negative, out of bounds, or
          // beginning > end
            perror (
                "[ \033[1;31mFAILED\033[0m ] print_v: requested position out "
                "of bounds");
            return;
        }
...
sum_v (vec_t *vec, size_t beg, size_t end)
{
    if (beg < 0 || end < 0 || beg > vec->size || end > vec->size || beg > end)
        { // if requested size is negative, out of bounds, or
          // beginning > end
            perror (
                "[ \033[1;31mFAILED\033[0m ] sum_v: requested position out of "
                "bounds");
            return 0;
        }
...
prod_v (vec_t *vec, size_t beg, size_t end)
{
    if (beg < 0 || end < 0 || beg > vec->size || end > vec->size || beg > end)
        { // if requested size is negative, out of bounds, or
          // beginning > end
            perror (
                "[ \033[1;31mFAILED\033[0m ] prod_v: requested position out "
                "of bounds");
            return 0;
        }
